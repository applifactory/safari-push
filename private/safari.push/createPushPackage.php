...
$certificate_path = "../aps.cert/web_aps_prod.p12";     // Change this to the path where your certificate is located
$certificate_password = getenv('SAFARI_PUSH_CERT_PASS'); // Change this to the certificate's import password
...
function copy_raw_push_package_files($package_dir) {
    ...
    foreach (raw_files() as $raw_file) {
        copy("$raw_file", "$package_dir/$raw_file");
    }
}
...
function create_signature($package_dir, $cert_path, $cert_password) {
    ...
    openssl_pkcs7_sign("$package_dir/manifest.json", $signature_path, $cert_data, $private_key, array(), PKCS7_BINARY | PKCS7_DETACHED, "../aps.cert/AppleWWDRCA.pem");
